...
让人不快的是，任意给定一套货币，判断它是否适用贪心法的方法并不简单。不过一般我们可以依据经验和寻找反例来进行判断，大概可以有下面这么几条规则：
...
3. 如果和通常的真实货币面值不同，特别是有一些奇怪的数值，例如7、11等，又构不成等比数列的就要小心了。遇到这样的数值可以用略高于那个奇怪的面值的数字来试试能不能找到反例。
