...
组合数的量级要比排列数小很多，但仍很大。若考虑 :math:`m` 为定值，:math:`n` 为数据规模，鉴于组合数的对称性，取 :math:`k=\min\{m,n-m\}`，组合数 :math:`C_n^m` 的量级为 :math:`O(n^k)`，为 :math:`k` 阶多项式级别。但是如果 :math:`m` 随着 :math:`n` 的增大而增大，事情就会迅速失控，甚至比排列数的情况更糟糕，会接近于幂指数级别。极限状态为 :math:`m` 始终等于 :math:`\lfloor{n\over2}\rfloor`，那就确确实实的成为了 :math:`O(n^{\lfloor{n\over2}\rfloor})`，恐怖的幂指数级别。C++最大的内置整型数仅能容纳到 :math:`C_{62}^{31}`。
