...
我们现在要用列表和递推的方法来求解这个函数。我们这样考虑，假如已经知道了从 ``S[i]`` 到 ``S[j]`` 这一段是不是回文串，那么对于“包围”它的那一段，即从 ``S[i-1]`` 到 ``S[j+1]`` 是不是也很容易判断了呢？当然很容易！如果 ``S[i]`` 到 ``S[j]`` 不是回文子串，那么包围它的那一段肯定也不会是回文子串；如果 ``S[i]`` 到 ``S[j]`` 是回文子串，那么包围它的那一段是不是回文子串取决于两头的字符 ``S[i-1]`` 和 ``S[j+1]`` 是不是相同。也就是说我们可以根据 :math:`P(i,j)` 的值轻松地推出 :math:`P(i-1,j+1)` 的值：
