...
正则表达式翻译成状态机有一套完备的方法，但是这套方法是比较复杂的，我们这里不打算详细说明，只想说明几个要点，其余的通过atoi问题这个实际的例子来获得一些感性认识即可。如果下面的过程看起来不太能懂的话，直接跳过去，看后面状态机的最终形态就好。
...
atoi问题的状态机会比前面的两个例子略微复杂一些，因为它现在还需要引入一些数据并且在状态转移的时候对这些数据进行必要的操作，以便到达终止状态的时候机器的数据是想要转换的那个正确的整数值。为了准确表示一个int型的整数值，我们需要在机器里加入两项数据：正负符号和值。可以用一个bool型变量 ``neg`` 来表示这个数是不是负数，初始值为false，如果字符串中第一个字符是符号 ``'-'`` 就把它改成true。另外用一个long long型变量 ``value`` 存放值，初始值为0。
...
1. 定义一个结构来表示状态机，其成员变量包括一个状态机的状态编号和机器本身的一些数据，比如这里要用到值和负数标志。
...
3. 一般建议在一个单独的函数中进行状态机的运行，这个函数接受两个参数，一个是要运行的状态机，另一个是信号。在这里就是一个 ``FSM`` 结构的变量和一个字符。在函数内部建议尽量多使用switch居于而非长长的if...else if...else if...。而且哪怕是atoi这样的简单状态机问题也不建议在main函数里直接编写状态机运行的代码。
...
对于atoi这样的简单问题，状态机的运行过程可以直接用switch语句或者if语句硬编码实现。但是对于比较复杂的问题，一般的方法是把转移和数据处理规则编写成指令表，通过查表的方式来进行，这种一般的方法需要用到更加高级的编程技术，比如面向对象、代理模式、委托模式等等，一般不在算法编程考查的范围之内。

.. admonition:: 练习

   仿照上面的atoi程序，自己编写一个完成16进制字符串转整数的程序axtoi，用于读取一个表示16进制整数的字符串并将其转为int型数值。
