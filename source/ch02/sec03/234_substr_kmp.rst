...
精确子串搜索有一个非常经典的算法，由算法界的三位大神D.E.Knuth、J.H.Morris和V.R.Pratt同时发明的。这个算法是每一本算法教科书都必讲、每一个学习算法的人都必学的超经典算法，就以三位发明者的姓氏首字母命名为KMP算法。它是迄今为止精确子串搜索最快的算法之一，也是算法史上最伟大的发明之一，它的时间复杂度是 :math:`O(n+M)`\ 。下面我们要来讲解一下这个KMP算法。
...
再重复一遍（因为这很重要），next数组和模式串长度相等，第 ``j`` 个元素 ``next[j]`` 表示前 ``j`` 个字符构成的子串 ``P[0:j]`` 的最长相同前后缀长度。我们已经知道，对于任何一个模式串，\ ``next[0]`` 都等于-1，\ ``next[1]`` 都等于0。一看到这种“有初值”的数值序列，大家一定要第一时间考虑“递推”这种重要的数学方法，快速计算next数组就是要用递推的方法。
...
理解了上面三种情况之后，就可以根据它们来归纳出从初值 ``next[0] == -1, next[1] == 0`` 开始推算整个next数组的算法了。但如果直接依据三种情况来写程序，代码会比较复杂。越是复杂的代码越容易有bug，所以实际编程时还要再改进一下。复杂在什么地方呢？复杂在算法的两个终止条件，情况1和情况2相差比较大，二者的判断条件和终止方式都不相同，我们应该设法让它们俩统一起来。

这里我们再次利用 ``next[0] == -1`` 来进行改进。遇到情况2的时候不要直接去终止循环，而是让它也按照情况3的方式继续循环下去，这会使 ``k = next[0] == -1``\ 。虽然我们本来的目标是想要让 ``next[j+1]`` 变成0，但-1加1不就是0嘛。所以可以给情况1加一个条件：当 ``k == -1`` 时也和情况1一样处理，令 ``next[j+1] = k + 1`` 并结束循环。是不是感觉和KMP算法主框架的技巧非常相似？下面就是这种朴素的next算法：
