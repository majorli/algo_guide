...
在后面的内容中我们先使用C-string来进行简单字符串处理，当对C-string及其常用操作有了一定的了解，能够阅读一些简单的程序代码的时候就将转为全面使用C++ string。这一节我们先来看三个字符串处理问题：周期串、子序列和Z字形变换。前两个问题来自《算法入门经典（第2版）》第三章的两个练习题，也是知名OJ网站UVa的题目，后一个问题来自著名的编程能力训练网站力扣（LeetCode）的算法类问题集。这三个问题的解法都很简单，但都包含了最基础的字符串处理技巧。
