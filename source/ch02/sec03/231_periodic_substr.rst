...
在后面的内容中我们先使用C-string来进行简单字符串处理，当对C-string及其常用操作有了一定的了解，能够阅读一些简单的程序代码的时候就将转为全面使用C++ string。这一节我们先来看两个非常简单的问题：周期串和子序列。这是《算法入门经典（第2版）》第三章的两个练习题，都是来自OJ网站UVa的题目。
