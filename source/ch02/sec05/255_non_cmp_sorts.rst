...
如果是单纯的计数排序，那么无所谓稳定不稳定，因为它很难实现对复杂结构数据的排序，也很难适应多关键字的排序。但是一般我们会采用一个技巧，计数结束后对计数数组做前缀和，使其值从单纯的计数值变成指示对应的元素在排完序的序列中所处的最后一个位置，然后逆序读取每一个待排序元素，按照计数数组中的位置写到一块新开辟的临时空间里去，这样处理完之后新的临时空间里存放的就是已经排好序的新序列。这样以来就变成了稳定排序，而且可以支持复杂数据结构元素的排序了。
...
:strong:`基数排序`\ 是桶排序的一种特殊变种，它一般用于整数排序，经过改造也可以用于浮点数、特定格式的字符串等进行排序，但很少见。
...
.. code-block:: none
   ...
   如果事先知道待排序数最大为三位数，那么到这里基数排序以经结束，可以看出原序列已经完全有序。
...
基数排序是利用了十进制整数每个数位上的数码只有0到9这九种的特性，通过从最低位到最高位依次进行多轮分桶读桶的操作，逐步实现排序。它的空间复杂度为 :math:`O(n)`，属于稳定排序，时间复杂度为 :math:`O(nL)`，其中 :math:`L` 为待排序元素的最大位数。这是一种相当优秀的时间效率，可惜适用场景比较狭窄。
...
下面就是一个使用数组和计数排序技巧的基数排序函数示例代码，可以对正整数进行基数排序。这里使用的计数排序技巧非常重要，一定要看懂、理解、学会！
