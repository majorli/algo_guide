...
根据中位数的定义，把 :math:`A,B` 两个数组有序归并成数组 :math:`C`，取 :math:`half = \lfloor(m + n) / 2\rfloor`，将 :math:`C` 分为前后两半。前半部分有 :math:`half` 个数，后半部分有 :math:`m + n - half` 个数。这样，前半部分中的最大值一定小于等于后半部分中的最小值。那么我们就已经找到中位数了，如果 :math:`m+n` 为奇数，那么中位数就等于后半部分的最小值；如果 :math:`m+n` 为偶数，那么中位数就是前半部分最大值和后半部分最小值的平均值。

有了上面的知识，我们就可以进一步这样考虑：怎样把所有数分成这样的前后两个部分，一共有多少种分发？从而可以引出新的二分算法来求中位数。

不失一般性，先让我们假设 :math:`m\le n`，即数组 :math:`A` 的元素个数不大于数组 :math:`B` 的元素个数。如果读入的数据情况相反，我们可以在编程的时候用一个小技巧把两个数组互换过来，这个技巧到后面再讲，现在先在这个假设下进行分析。

前面所述的将归并后的数组 :math:`C` 分为前后两部分，实际上在它分成的前一部分，一定有一些数是来自于数组 :math:`A` 的，另一些数是来自数组 :math:`B` 的，在分成的后半部分也一样。也就是说，数组 :math:`A` 和 :math:`B` 都为最终的前半部分 :math:`C` 和后半部分 :math:`C` 贡献了一些数。而且不难想象，数组 :math:`A` 贡献给前半部分的数一定都不大于它贡献给后半部分的数，即 :math:`A` 对前后两个部分的贡献其实也是它自己的一个前后划分。这对于数组 :math:`B` 也是同样的情况。所以，是数组 :math:`A` 和 :math:`B` 各自的一个前后分割，组合成了 :math:`C` 的前后两个部分。

于是我们可以考虑先任意分割数组 :math:`A`，即把它任意分割成前后两个部分，设前一部分一共 :math:`k` 个元素，那么后一部分就是 :math:`m-k` 个元素，而且分割点 :math:`k` 的取值范围为 :math:`0\le k \le m`。即数组 :math:`A` 给 :math:`C` 的前半部分贡献其前 :math:`k` 个数，给 :math:`C` 的后半部分贡献后 :math:`m-k` 个数。当 :math:`k=0` 时，表示数组 :math:`A` 中所有元素最终都贡献给了 :math:`C` 的后半部分；当 :math:`k=m` 时，则全部贡献给了 :math:`C` 的前半部分。

数组 :math:`A` 划分完毕后，实际上数组 :math:`B` 的划分也就依此而确定了。这是因为 :math:`C` 的前半部分规定了是要有 :math:`half` 个元素，所以既然 :math:`A` 已经提供了 :math:`k` 个，那么 :math:`B` 就一定是要提供 :math:`half-k` 个。所以我们接下来要给数组 :math:`B` 做的划分已经确定下来了：数组 :math:`B` 的前 :math:`half-k` 个元素贡献给 :math:`C` 的前半部分，剩余的 :math:`n-(half-k)` 个元素贡献给 :math:`C` 的后半部分。因为我们已经限定了 :math:`m\le n`，所以有 :math:`m\le half \le n`，又因为有 :math:`0\le k\le m`，所以我们可以确保 :math:`B` 的划分一定不会“出界”，即 :math:`0\le half-k\le n` 得到了保证。这就是我们前面要限制 :math:`A` 数组比较短的原因。

.. admonition:: 补充

   实际上，如果我们不限定 :math:`m\le n` 也是可以分析的，当 :math:`m \gt n` 时，数组 :math:`A` 贡献给前半部分的元素数量 :math:`k` 会有一个不同的取值区间，以确保对数组 :math:`B` 的划分不会出界。这个分析会相对比较麻烦一些，会烧脑，而且我们知道，烧脑容易带来错误，所以我们干脆限定 :math:`m\le n` 更好。

现在，问题的规模就限制在了对数组 :math:`A` 的总共 :math:`m+1` 中划分之内，分别为 :math:`k = 0,1,2,\dots,m`，其中必有一种是满足数组 :math:`C` 的中位数划分的，即前半部分的最大值小于等于后半部分的最小值。我们的任务变成了单纯地找出这种划分，如果蛮力枚举，也已经把时间复杂度降到了 :math:`O(\min\{m,n\})`。如果我们对于任意一种划分 :math:`k`，有办法确定它是大了还是小了，即数组 :math:`A` 给 :math:`C` 的前半部分贡献其前 :math:`k` 个元素是给多了还是给少了，那么我们就可以愉快地使用二分法来查找正确的划分数 :math:`k` 了，左端点为 :math:`0`，右端点为 :math:`m`，时间复杂度降到 :math:`O(\log\min\{m,n\})`，问题就能完美解决了。

其实要做到判断一个划分 :math:`k` 是大了还是小了还是正好，是很简单的，只要对 :math:`C` 前半部分的最大值和后半部分的最小值进行对比分析即可。那么我们来分析一下，一共有三种情况：

1. :math:`1\le k \le m-1`，这时候 :math:`A[0]` 到 :math:`A[k-1]` 贡献给了 :math:`C` 的前半部分，:math:`A[k]` 到 :math:`A[m-1]` 在后半部分。相应的，数组 :math:`B` 的划分点为 :math:`j=half-k`，满足 :math:`1\le j\le n-1`，所以 :math:`B[0]` 到 :math:`B[j-1]` 贡献给了 :math:`C` 的前半部分，后半部分则是 :math:`B[j]` 到 :math:`B[n-1]`。两个原数组对 :math:`C` 的前后两半都有贡献，如下所示：
   ...
   1. 当 :math:`A[k-1]\gt B[j]` 时，说明数组 :math:`A` 贡献给 :math:`C` 前半部分的数太多了，有过大的数混进前半部分去了，所以这时候我们应该减小 :math:`k` 的值；
...
2. :math:`k=0` 时，这时候数组 :math:`A` 中所有数都分在了 :math:`C` 的后半部分，:math:`C` 的前半部分全部由 :math:`B` 的前 :math:`j` 个数构成。此时 :math:`j=half`，当 :math:`m=n` 时 :math:`j=m=n`，会出现整个数组 :math:`B` 都在 :math:`C` 前半部分的情况，但是这没有关系，如下所示：
   ...
   现在，数组 :math:`A` 对 :math:`C` 前半部分没有贡献任何数，所以前半部分的最大值就是 :math:`B[j-1]`，而且 :math:`A` 的划分数 :math:`k` 也已经不能再减小了。因此只能有两种情况，要么 :math:`k` 小了，要么这个划分正好。而 :math:`B` 有没有对 :math:`C` 的后半部分贡献元素是无关紧要的，因为要减小 :math:`k` 的情况不存在，而且反正也没有数需要和它比大小：

   1. 当 :math:`B[j-1]\gt A[k]` 时，说明数组 :math:`A` 贡献给 :math:`C` 前半部分的数太少了，有过小的数混进后半部分去了，所以这时候我们应该增大 :math:`k` 的值；
   2. 其余情况，说明这就是中位数的正确划分。

3. :math:`k=m` 时，这时候数组 :math:`A` 中所有数都被分在了 :math:`C` 的前半部分，:math:`C` 的后半部分全部由 :math:`B` 的后 :math:`n+m-half` 个数构成。此时 :math:`j=half-m`，当 :math:`m=n` 时 :math:`j=0`，会出现整个数组 :math:`B` 都在 :math:`C` 后半部分的情况，但是和情况2类似，这也没有关系，如下所示：
   ...
   现在，数组 :math:`A` 对 :math:`C` 后半部分没有贡献任何数，所以后半部分的最小值就是 :math:`B[j]`，而且 :math:`A` 的划分数 :math:`k` 也已经不能再增大了。因此只能有两种情况，要么 :math:`k` 大了，要么这个划分正好。而 :math:`B` 有没有对 :math:`C` 的前半部分贡献元素是无关紧要的，因为要增大 :math:`k` 的情况不存在，而且反正也没有数需要和它比大小：
...
上面就是整个二分查找数组 :math:`A` 的合适划分数 :math:`k` 的过程，当查找到正确的划分后，我们只要把 :math:`C` 前半部分的最大值 :math:`\max_h` 和后半部分的最小值 :math:`\min_r` 计算出来，结束查找就可以了。在结束查找之后，根据 :math:`m+n` 的奇偶性得出中位数。如果 :math:`m+n` 为奇数，那么中位数等于 :math:`\min_r`；为偶则中位数等于 :math:`\max_h` 和 :math:`\min_r` 的平均值。
...
最后在动手编程之前，讲解一下怎样确保两个原数组一定是 :math:`m\le n`。当然不是读完之后判断二者大小然后交换数组元素，那太笨了。我们可以定义一个二维数组用来保存两个原数组：
