...
.. note::
   ...
   这段代码的循环条件值得关注，它使用了一个自除10表达式 ``n /= 10``，这个表达式的值是n除以10之后得到的商。这样做使得程序不需要在循环体内去完成n的自除，所以循环体内只需要一条修正结果的语句就够了。之所以能这么做是因为我们把结果的初始值设为了原数的个位数，而不是很多整数反转算法里采用的初始值0。请想一想如果我们把结果的初始值设为0，这个算法和代码要怎么改变？
