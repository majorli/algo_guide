...
既然原因是5到10\ :superscript:`8`\ 这个数据范围太大导致埃筛在初期速度略慢，那么我们有没有什么办法可以缩小这个范围呢？办法是有的。我们在前面讨论回文质数的时候说过，偶数位的回文数除了11以外全部是合数。而10\ :superscript:`8`\ 以内最大的奇数位回文为9999999，其次大的是9989899。我们可能一时无法确定9989899是不是质数，但是9999999肯定不是质数。所以我们至少可以把数据的最大值限制在9989899封顶的范围内，这就比一亿小了10倍，值得一试。

.. literalinclude:: ../../codes/214_p1217_ER.cpp
   :language: c++
   :emphasize-lines: 14

只需要加一句语句，然后在洛谷网站测试一下，结果全部测试点AC。从测试点的运行时间来看，也说明了当数据规模的数量级从10\ :superscript:`7`\ 增长到10\ :superscript:`8`\ 后埃筛的速度降幅高达4至5倍。
...
进一步考量回文质数除11以外全部是奇数位的性质，我们可以发现在5到100的范围内，其实只有3个回文质数：5、7、11。如果我们事先把这三个数特判掉，那么剩下的回文质数可选范围就只可能有三位数、五位数和七位数了。
