...
对于大于1的奇数幂，我们可以先提出一个底数的1次方，然后把剩下的偶数幂部分按上面的方法进行降幂。所以一轮降幂的过程可以用下面这个数学公式来表示：
...
.. math::
   ...
   1&,n=0\\
   a&,n=1\\
   (a^2)^{n\over2}&,n=2,4,6,\dots\\
   a\cdot(a^2)^{\frac{n-1}{2}}&,n=3,5,7,\dots
   \end{cases}
...
1. 第1轮循环，指数13大于1而且是奇数，所以在第一轮循环的时候我们拆出一个底数3来乘到结果变量ans上去，使得ans=3，然后指数除2变为6，底数平方变为9。
2. 第2轮循环，指数6大于1，是偶数，所以ans不变，指数除2变成3，底数平方变成81。
3. 第3轮循环，指数3大于1，是奇数，所以拆出一个底数81乘到ans上去，使得ans=243，然后指数除2变成1，底数平方变成6561。
4. 第4轮循环，指数为1，不满足循环条件，循环结束。
5. 循环结束，把底数乘到ans上去，使得ans=1594323。

仔细观察得出结果的过程可以发现，结果ans的值来自于3、81、6561三数的相乘，实际上就是\ :math:`3^1\times3^4\times3^8`\ 。所以二分降幂法和指数拆分法本质上其实没有什么区别，可以说是异曲同工，最终殊途同归的两种实现方式而已，这也是为什么我们把这两种方式都归为同一种算法（都是整数快速幂算法）的原因。

在编写这个方法的代码时有一个非常容易疏忽的出错点，由于计算时会不断地对底数进行平方，所以底数很容易发生溢出爆掉。因此在代码中要把底数复制到一个数据类型比它更大的临时变量里去，用这个体型更大的替身来代替原来的底数进行运算。下面是二分降幂法快速幂的代码：
...
.. admonition:: 模m运算
   ...
   在数学里，模m运算是普通算术运算的一种加以限制的变种，简单地说可以理解为所有参与运算的数全部取除m的余数的四则运算。其中m称为\ :strong:`模`\ ，是大于等于2的整数。例如在模7运算里，所有参与运算的数和运算结果都应该在0到6的范围内。例如：普通意义上的7在模7运算时就应该等于0，因为7除7的余数为0。在此意义下，3+2仍然等于5，因为5除以7的余数仍然为5，但是3+5就应该等于1，因为8除7的余数为1。
