...
除了用类似数组的方括号加下标的方式来访问元素，vector（包括所有其他STL容器）还提供了一种叫做\ :strong:`迭代器`\ （iterator）的东西来指定表内元素的位置。容器本身提供一头一尾两个迭代器，分别用 ``begin()`` 和 ``end()`` 两个成员函数来获得。头迭代器指向第一个元素，也就是0号元素；尾迭代器指向紧跟在最后一个元素后面的那个位置；迭代器可以使用加减运算来改变它的位置；尾迭代器减头迭代器得到的差等于容器内的元素个数。
