...
二叉树最常见的实现方式为链式结构。根据二叉树的递归定义，我们只需要这样一种节点结构就可以实现二叉树了：可以存放一个数据项，有两个指向左右子树的指针，指针类型就是节点结构本身。比如下面这个简单的程序定义了一个元素数据类型为 ``int`` 的简单的二叉树节点结构，在 ``main()`` 函数中生成了一个根节点（也就是生成了一棵新的二叉树），根节点数据初始值为4。随后将根节点的数据值加1。运行这段程序会先后输出4和5：

.. literalinclude:: ../../codes/341_bitree_1.cpp
   :language: c++

这里这个简单的 ``Node`` 结构我们将在后文的示例程序中一直继续使用。
