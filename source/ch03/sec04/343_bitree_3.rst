...
要实现这样一个先序遍历的算法程序可以说是再简单不过了，我们继续沿用数据项为整数的示例，下面是树节点的结构类型定义：

.. literalinclude:: ../../codes/343_bitree_trav.cpp
   :language: c++
   :lines: 7-15

我们可以用下面这样的代码来构造出上例中的二叉树，当然也可以用类似的方法来自己构造一些不同形状的二叉树用于测试：

.. literalinclude:: ../../codes/343_bitree_trav.cpp
   :language: c++
   :lines: 30-39

而利用递归调用来实现的先序遍历算法则非常非常地简单（假设我们的访问方式只是输出节点中的数据）：
...
尽管很少会有一棵真实存在的二叉树大到能导致Stack Overflow的程度，但如果是在DPS算法中就不一定了，DPS算法要搜索的树往往不是真实存在的树，而是随着搜索不断生长的虚拟树。如果用递归的方法来进行DPS，发生栈溢出还是很有可能的，何况运行速度也会受影响。
...
实现非递归调用版本的先序遍历（或DPS），要用到一个栈来作为辅助。

.. hint::

   在算法领域有一条非常具有普适性的规律：递推性质的过程在实现的时候常常用到队列，例如层层推进的层序遍历；递归的过程则往往利用栈，例如二叉树的先序遍历。

先序搜索所使用的栈，和层序搜索使用的队列作用是一样的，就是用来存放接下来要遍历的子树的根。每一次从栈从取出栈顶元素所指向的节点，它就是接下来要遍历的子树的根节点。依照先序遍历的规则，根节点先访问，所以我们将栈顶元素弹出后自然是先访问它所指向的节点，然后依次将右子节点和左子节点压栈，它们就是接下来要遍历的子树的树根。

.. warning::

   由于先序遍历的顺序是“根、左、右”，即根节点访问之后先访问左子树，而栈是后入先出的，所以在压栈的时候要将顺序反过来，先压右儿子，再压左儿子。

下面先看示例，刚才这棵二叉树，现在我们用非递归调用的方法来对它进行先序遍历，第一步先将整棵树的根节点压栈，如下图：

.. image:: ../../images/343_pre_ord_norec_1.png

接下来就不断重复前面所述的步骤，从栈顶元素所指的节点开始，边访问，边压栈，直到栈弹空为止，先序遍历就完成了。

.. image:: ../../images/343_pre_ord_norec_2.png

.. image:: ../../images/343_pre_ord_norec_3.png

下面这个算法虽然是二叉树先序遍历的非递归调用版算法，但也是所有DPS算法的基本模式：
