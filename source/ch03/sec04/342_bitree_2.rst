...
现在就有了一个严重的问题，我们的二叉树中所有的节点都是通过 ``new`` 语句使用动态内存技术分配出来的，所以如果要删除一个节点时就应该用 ``delete`` 语句来释放掉它占用的动态内存。这就是动态内存管理中最重要最著名的有借有还原则。
...
.. image:: ../../images/342_delete_1.png

现在开始下面这样的循环：将队头所指节点的子节点从左到右依次入队，然后销毁队头所指节点，释放其占用的内存，并进行一次出队。第1轮循环完成后的情况如下图：

.. image:: ../../images/342_delete_2.png

下一轮循环将要销毁的队头所指节点为节点2，在图中用红色表示。第2轮循环完成后变成下图的样子：

.. image:: ../../images/342_delete_3.png

随后不断循环直到队列为空，整棵树的销毁过程也就结束了：

.. image:: ../../images/342_delete_4.png

下面这个函数用来销毁一棵二叉树。这里用到一个奇怪的小技巧：指针的指针。
...
指针的指针，顾名思义，终究还是一个指针，只不过这个指针所指向的实际变量还是一个指针。这种二重指针在软件开发中是很常用的，但是在算法程序中很少用到，甚至要避免使用，因为它太容易让人混乱了。那为什么这里我们要用到它呢？因为C++动态内存技术有一条规范，凡是 ``delete`` 掉的空间，应该马上把原先指向它的指针设置为空。这是因为 ``delete`` 命令只负责释放掉指针所指向的动态内存空间，并不会改变指针的原值，空间释放之后的指针其实还是指向原来的地址的。为了避免误使用已经被释放掉的内存块，我们应该在 ``delete`` 后及时把原来的指针置为空。而在一个函数内如果要修改参数的实际值，就要用传引用或者传指针的形式来传递这个参数，如果这个参数本身是指针，那么更加顺理成章的就是用传指针的方式来传指针参数，即参数采用指针的指针的形式。


.. admonition:: 练习

   认真看懂本节所给出的节点结构和三个操作函数，然后自己编写 ``main()`` 函数，实现以下功能：

   1. 输入一个整数 :math:`6 \lt n \lt 100`\ ，然后用前 :math:`n` 个正整数构造一棵完全二叉树。
   2. 按层序输出这棵二叉树中所有节点上的数据。
   3. 销毁根节点的左子节点的整棵右子树，然后再次层序输出此二叉树。
