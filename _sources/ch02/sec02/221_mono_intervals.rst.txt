...
如果数列 :math:`A` 的一段 :math:`\{a_i,\dots,a_j\}` 中每一项都比它前面的项更大，即对于任意的整数 :math:`i\lt k \le j`\ ，都有 :math:`a_k \gt a_{k-1}` 成立，那么数学上就把这一段称为数列的一个单调递增区间，长度为 :math:`j-i+1`\ ；反之如果这一段里的所有项都比它前面的项更小，它就是数列的一个单调递减区间，长度当然也是 :math:`j-i+1`\ 。
