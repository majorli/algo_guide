...
网上介绍最多的一个版本是以序列的首元素为基准，从两头开始向中间逐个比较元素，先从后向前找第一个小于基准值的元素将其放到前面，然后从前向后找第一个比基准值大的元素将其放到后面，不断循环这个过程直到前后两个查找位置相遇或交错而过后，把基准值插到中间就完成了。这是网上流传最广的分区方法，有许多详解教程，许多算法教材也用这种方法，大家可以自己去搜索相关资料学习。
...
.. attention::
   ...
   如果大家试着去看过《算法导论》，或者在网站上看过对该书中快速排序算法的解读，会发现在《算法导论》中所有的序列都是从1开始计数的。这是因为该书是脱离具体的编程语言而纯粹讲解算法的，书中没有任何具体的程序，都是高度抽象的伪代码。但是我们现在使用C++语言学习算法，所以改成符合C++习惯的从0开始计数，序列范围用含头不含尾规则来表示。
