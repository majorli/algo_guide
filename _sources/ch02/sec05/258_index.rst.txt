...
:strong:`建索引`\ 就是不改变原序列，而是为每一个元素生成一个整数值，表示如果对序列进行排序的话，这个元素将被放置到的位置，称为元素的\ :strong:`索引`。如果索引从1开始编号的话，元素的索引值也就是这个元素在序列中排第几小的序号，索引为1的元素是最小的元素，索引为k的元素是就是序列的k位数。当然在C++语言中容器和数组的元素位置都是从0开始编号的，所以k位数的索引应该是k-1。

通常，对于长度为 :math:`n` 的序列 :math:`A`，元素的索引值就取从 :math:`0` 到 :math:`n-1` 的整数，元素的索引值都是唯一的，互不相同。算法会生成一个新的长度为 :math:`n` 的整数序列 :math:`K` 用来存放所有元素的索引值，:math:`K[i]` 存放元素 :math:`A[i]` 的索引，表示如果对序列 :math:`A` 进行排序的话，位于位置 :math:`i` 的元素 :math:`A[i]` 排序后的新位置将是 :math:`K[i]`。这个新的序列 :math:`K` 称为 :math:`A` 的\ :strong:`索引表`。建索引算法的任务就是为一个序列按照一定的排序规则建立索引表，但不改变原序列。

建索引的算法通常需要用到一些高级数据结构，最常见的是用B树、B+树等检索树结构，是属于比较复杂的算法。但是对于一些特殊场景，也有一些简单的建索引算法，不需要用到高级数据结构就可以实现，但是在时间或者空间上就会有所限制。这一节我们将介绍几种这样的简单索引算法。
...
这种方法在排序前要进行一次数据加下标的包装，复制到一个临时数组里，这是 :math:`O(n)` 时间的操作，排序后建立索引表的过程也是 :math:`O(n)` 的，所以整个过程的时间复杂度取决于对临时数组的排序时间。若采用C++算法库的不稳定排序函数 ``std::sort()`` 则约为 :math:`O(n\log n)`，若采用其稳定排序函数 ``std::stable_sort()`` 则大约在 :math:`O(n\log n)` 到 :math:`O(n\log^2 n)` 之间。从时间效率来看还是不错的。但是它的空间复杂度比较大，需要 :math:`O(n)` 的额外空间占用。
...
2. 元素0，在原序列中下标为1，在它之前有1个元素比它大，因此如果排序，它首先要向前移动1个位置，以便把它前面的那个逆序数放到它后面去，在它后面没有比它小的逆序元素，所以元素1的索引为 :math:`1-1=0`。
...
这个算法，空间复杂度为 :math:`O(K)`，其中 :math:`K` 为取值范围的宽度（现在索引表占用的一个 :math:`O(n)` 的空间已经不是临时空间而是所需要的算法结果了，所以不能算入空间复杂度里去）。时间复杂度为 :math:`O(n+K)`，当然了它是指数时间，不是线性时间这一点一定要搞明白，当 :math:`K` 很大时就不太好用了，超过100万会突然变得奇慢无比而且空间占用极大，很容易造成空间耗尽的MLE错误。当有关键字重复的元素时，它排出来的索引是具有稳定性的。对于符合计数排序条件的合适的问题，这是一个实际运行非常快速的索引算法，是很实用的。

上面所述的三种比较简单，不需要用到复杂的数据结构的建索引算法。虽然各有各的局限性，但它们有一个共同的优点就是简单，无论是设计思想还是编程实现都不难，在实际问题中也有不少场景适用，所以也是非常重要的基础算法。
...
.. admonition:: 练习
   ...
   输入格式：第一行一个整数 :math:`n`，范围为 :math:`0\lt n \lt 1000`，后面 :math:`n` 个整数，每个一行，依次为学号1号到 :math:`n` 号的学生的分数，按学号顺序排列。
