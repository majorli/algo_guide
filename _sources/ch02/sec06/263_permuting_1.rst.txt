...
.. hint::
   ...
   上面这个方法是最为实用的生成 :math:`n` 选2排列的方法，长度为2的排列就这么干，不用考虑更复杂的算法。
...
先解决内层循环变量避开外层循环变量的判断问题。我们知道，循环变量就是前 :math:`n` 个元素的索引，从 :math:`0` 到 :math:`n-1`\ 。根据不可重复排列的性质，已经被选中过的元素就不能再次被选中，即已经选中过的元素的索引不能再次被使用。所以我们可以事先设置一个表示有没有被选中过的逻辑型标志数组，长度为 :math:`n`\ ，对应从 :math:`0` 到 :math:`n-1` 号元素的索引，某个元素被选中的时候就把对应的标志设为 ``true``\ ，那么每一层循环里只要判断当前循环变量对应的标志是不是 ``true`` 就可以了。当排列发生变化，某一层循环将要进入下一轮（即该位置上的元素将要发生变化）时，别忘了要及时把当前选中元素的标志改回 ``false``\，以便其他位置可以选它。经过这样的修改，上面的代码就变成了下面这样：
...
由于这个算法是在 :math:`m` 被减到 :math:`0` 的时候调用消费者函数的，这时候光看 :math:`n` 和 :math:`m` 的值并不能知道排列的长度，所以我们还需要有一个计数变量 :math:`len` 表示一共取了几个元素，第一次调用的时候 :math:`len=0`\ ，表示还没有取过元素。以后每取完一个元素递归进入下一层的时候就传递 :math:`len+1` 的值，这样当 :math:`m` 被逐一减为0的时候 :math:`len` 就被逐一加到了 :math:`m`\ 。
