...
这里有一个需要注意的地方，题目并没有说输入数据中的交头接耳学生对坐标一定是左前右后的或下前上后的。换句话说，对于一左一右的情况，只能知道两位学生的行坐标相同 :math:`X=P`\ ，但不能保证 :math:`Y\lt Q`\ ，因此不能简单地以为此时通道编号就是 :math:`Y` 号列通道，而应该是是 :math:`\min\{Y,Q\}` 号列通道。对于一上一下的情况也是类似。在编程的时候可以使用 ``algorithm`` 库的 ``min()`` 函数来取最小值，不需要自己写判断语句或者三元表达式了，这是C++语言编程所推荐的方式。
...
现在就可以对行通道数组的 :math:`[1,M)` 段、列通道数组的 :math:`[1,N)` 段进行排序了，使用 ``algorithm`` 库的 ``sort()`` 函数排序即可，这里不需要稳定性。
