...
解决最大回文子串问题的方法有很多，我们先通过蛮力法来了解一些基本知识，然后学习一种比较容易理解和编程的方法：中心扩展法，它的时间复杂度是 :math:`O(n^2)`\ 。由于中心扩展法编程比较简单，不使用什么复杂的数据结构，消耗空间仅为常数级的 :math:`O(1)`\ ，时间上也能满足绝大多数题目的要求，所以是非常好用的一种算法，需要认真学习掌握。

最大回文子串问题的另一个 :math:`O(n^2)` 时间算法是一种动态规划法，但根据在LeetCode网站实测的结果，它的实际运行时间是中心扩展法的20多倍，而且空间复杂度同样高达 :math:`O(n^2)`\ ，因此我们只打算对它进行一个简单的原理介绍，对五大算法设计方法中最灵活最难掌握的动态规划法有一个初步的感性认识即可。

而最经典的最大回文子串问题解法是一个叫做Manacher算法的方法，通常习惯用谐音称呼它为\ :strong:`马拉车算法`\ 。这个算法非常高明，非常精妙，时间复杂度低至 :math:`O(n)`\ ，但是这个算法非常复杂，非常难掌握。不太指望能在考场上又快又正确地写出马拉车算法程序来，所以我们将在后面的章节再详细介绍马拉车算法，现阶段能熟练掌握中心扩展算法即可。
...
.. admonition:: 补充
   ...
   不过在实际应用中完全不用担心这一点点的速度差距。在现代计算机芯片的强大运算能力和现代C++编译器的强大优化能力下，这点速度差异微不足道，尤其是在使用C-string这种本身就以速度见长的表示方式时，二者几乎没有任何差别。除非字符串的长度非常巨大，数十甚至数百万位。
...
暴力枚举需要用到两层嵌套循环。设S的长度为n，外层循环是子串长度L，从n到1的逆向循环。内层循环是从左到右所有具有长度L的子串，即从 ``S[0:L]`` 到 ``S[n-L:n]``\ 。在内层循环里逐个对这些子串进行回文串判断，只要找到一个就是答案，就直接退出所有循环。显然这是一个 :math:`O(n^3)` 的算法。
...
对于任意一个长度为n的非空字符串S，首先我们定义一个 ``bool`` 型函数 :math:`P(i,j)`\ ：

.. math::

   P(i,j)=\begin{cases}\text{true}&\text{如果 }S[i] ~ S[j]\text{ 是回文子串}\\ \text{false}&\text{如果 }S[i] ~ [j]\text{ 不是回文子串}\end{cases},(0 \le i \le j \le n-1)

如果我们能够对所有的 :math:`0 \le i \le j \le n-1` 计算出 :math:`P(i,j)` 的值，那么当然就能求出最长回文子串了。但是使用暴力枚举的方法来计算所有 :math:`P(i,j)` 的值是不行的，因为这正是蛮力法所做的事情。

我们现在要用列表和递推的方法来求解这个函数。我们这样考虑，假如已经知道了从 ``S[i]`` 到 ``S[j]`` 这一段是不是回文串，那么对于包围它的那一段，即从 ``S[i-1]`` 到 ``S[j+1]`` 是不是也很容易判断了呢？当然很容易！如果 ``S[i]`` 到 ``S[j]`` 不是回文子串，那么包围它的那一段肯定也不会是回文子串；如果 ``S[i]`` 到 ``S[j]`` 是回文子串，那么包围它的那一段是不是回文子串取决于两头的字符 ``S[i-1]`` 和 ``S[j+1]`` 是不是相同。也就是说我们可以根据 :math:`P(i,j)` 的值轻松地推出 :math:`P(i-1,j+1)` 的值：
...
注意这里要满足 :math:`j-1\ge i+1`\ ，即 :math:`j-i\ge2`\ ，子串长度需要大于2。也就是说，像 :math:`P(i,i)` 和 :math:`P(i,i+1)` 这样的函数值不能用这个递推公式计算，它们需要作为函数的初始值实现计算好。幸亏长度为1和2的子串是否回文串很容易判断，所有长度为1的单字符子串都是回文，而长度为2的子串可以通过一次循环很轻松的判断完成。
...
表中对角线上的函数值都为true，对角线左下方的三角形区域是没有用的，那里 :math:`i\gt j`\ ，我们需要求的是对角线右上方的三角区域里所有的值。还可以发现，这个表里每一条斜线上的所有函数值对应的子串长度都是相等的，对角线上都是长度为1的单字符子串，而右上角的 :math:`P(0,n-1)` 则代表原字符串S本身。
...
为了熟悉上述过程，可以用几个短一点的字符串（长度10个字符以内）手动计算一下这张表格，有兴趣自己编一下程序就更好了。

.. hint::

   在正规的动态规划法术语里，这种可以由问题的简单情况解来逐步推出更复杂情况下的解的函数叫做\ :strong:`优化函数`\ ，最简单情况下的解叫做优化函数的\ :strong:`初值`\ ，程序里一般都用数组来完成列表计算，这样的表叫做\ :strong:`动态规划表`\ 或\ :strong:`备忘录`\ 。
...
算法的主流程就只需要从头到尾的遍历每一个字符 ``S[i]``\ ，并对它先后进行两次扩展：一次调用 :math:`\text{Expand}(S,i,i)` 进行以该字符本身为中心的扩展，另一次调用 :math:`\text{Expand}(S,i,i+1)` 进行以它和它后一个字符之间的间隙为中心的扩展。

.. hint::

   对于最后一个字符 ``S[n-1]`` 的情况，也可以放心地调用 :math:`\text{Expand}(S,i,i+1)`\ ，因为此时右指针已经超过限制，所以中心扩展子算法的循环根本不会进入，直接会返回 ``n - (n-1) - 1 = 0``\ 。

先后两次扩展，会得到先后两个长度，为了得到对应的子串，我们还需要根据长度来推算出子串的起始位置。首先我们分情况进行考虑：

1、以字符 ``S[i]`` 为中心进行扩展时，设得到的回文子串长度为 :math:`l_1`\ ，它一定为奇数且至少为1，可以表示为 :math:`l_1=2k+1`\ ，其中 :math:`k` 是0或正整数。子串应该是在 ``S[i]`` 前后各有 :math:`k` 个字符，像下面这个样子：
...
它的左端字符位置为 :math:`i-k=i-\frac{l_1-1}{2}`\ 。

2、以字符 ``S[i]`` 和 ``S[i+1]`` 之间的间隙为中心进行扩展时，设得到的回文子串长度为 :math:`l_2`\ ，它是一个偶数或零，即 :math:`l_2=2k`\ ，:math:`k` 也是0或正整数。子串应该是从 ``S[i]`` 开始向前有 :math:`k` 个字符，从 ``S[i+1]`` 开始向后也有 :math:`k` 个字符的样子：
...
左端字符位置为 :math:`i-k+1=i-(\frac{l_2}{2}-1)`\ 。

在数学上这是两个不同的公式，但是在用C++语言编程的时候，却可以利用整数除法的性质统一起来。我们知道C++的整数除法是直接删除小数部分进行取整的，即向下取整。所以对于偶数 :math:`l_2`\ ，如果用C++整数除法来计算 :math:`\frac{l_2-1}{2}`\ ，结果恰等于 :math:`\frac{l_2}{2}-1`\ ：

.. math::

   \left\lfloor\frac{l_2-1}{2}\right\rfloor=\left\lfloor\frac{2k-1}{2}\right\rfloor=\left\lfloor\frac{2(k-1)+1}{2}\right\rfloor=\lfloor(k-1)+0.5\rfloor=k-1=\frac{l_2}{2}-1

于是我们就可把两种情况的公式统一为 :math:`i-\frac{L-1}{2}`\ ，其中 :math:`L` 是扩展返回的回文子串长度。于是我们在循环中对每一个字符位置 ``i`` 进行两次中心扩展后，得到的两个长度中只需抛弃较小者，选其较大者参与最大长度的竞争即可，这无非就是找最大值这种简单的事情了。循环完毕最长回文子串就得到了。
...
.. attention::
   ...
   当然在最坏情况（所有字符全部相同，比如 ``"aaaaaa"``\ ）而且字符串长度很长时，二者还是有速度差异的。但是马拉车算法太难记住太难考场盲写了，所以通常来说还是中心扩展算法更加实用。在普及、提高这两个阶段，也很少出现非用马拉车算法不可的题目。所以请务必牢牢掌握中心扩展算法并且能自行编程实现。
