...
:strong:`周期串`\ 是指这样一种字符串，它是由某一个非空字符串重复若干次之后得到的。例如 ``"abcabcabcabc"`` 就是一个周期串，它是由子串 ``"abc"`` 重复4次之后得到的，也可以认为是由 ``"abcabc"`` 重复2次得到的。:strong:`周期`\ 是指被重复的那个短字符串的长度，所以一个周期串可能有多个周期。

例如前面举的这个例子里，如果把它看作是 ``"abc"`` 重复4次，那么它的周期就是3；如果看作是 ``"abcabc"`` 重复2次，那么周期就是6。所有周期中最小的那个叫做\ :strong:`最小正周期`\ 。
...
一个字符串 :math:`s`\ ，若其长度为 :math:`n`\ 。那么它的周期可能有哪些呢？显然周期的取值范围一定是在整数区间 :math:`[1, n]` 内的，而且这个周期一定能被 :math:`n` 整除。换句话说，周期一定是长度的因数。并且除了 :math:`n` 本身以外，其他的周期值都不会超过 :math:`n` 的一半。所以我们只需要从1开始逐个检查不大于 :math:`n\over2` 的因数是不是周期，第一个找到是周期的因数就是最小正周期。如果找不到，那么最小正周期就是 :math:`n`\ 。
...
测试的原理也很简单，我们只要对原字符串 :math:`s` 的所有长度为 :math:`p` 的分段进行逐个比较是否相等即可，算法如下（注意，字符串的范围一律采用含头不含尾的原则表示）：
...
要解决这个问题，我们需要分别遍历目标字符串 :math:`s` 和源字符串 :math:`t` 的每一个字符来进行“不走回头路”的比对。用两个下标 :math:`i` 和 :math:`j` 分别表示 :math:`s` 和 :math:`t` 当前被访问的字符位置，二者都从0开始直到各自字符串的结尾为止，逐个字符地向后行进，任何时候都不会往回走。
