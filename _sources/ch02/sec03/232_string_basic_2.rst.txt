...
.. admonition:: 练习
   ...
   请大家思考一下，假如在干草堆中不止有一根针，如何利用 ``strstr()`` 函数把所有这些针的位置都依次找出来？
...
.. admonition:: 练习
   ...
   这个题目还有另一种 :math:`O(n)` 时间的算法，它不记录每一个字符上一次出现的位置，而是记录它出现的次数。当某个字符第一次出现重复的时候，根据一定的规则去调整 ``i`` 和 ``j`` 的值。这个算法在思路上和我们用的算法是一致的，都是滑动窗口的思路，只是在实现方式上略有改变。请大家动脑筋想一想，自己设计出这种算法并编程验证。
