...
我们可以发现，每一个位置上的节点编号和它的父节点编号是有确定的关系的。对于任意一个编号为 :math:`n` 的节点：

1. 父节点编号：:math:`\text{Parent}(n)=\left\lfloor\frac{n-1}{2}\right\rfloor`\ 。
2. 左子节点编号：:math:`\text{LeftChild}(n)=2n+1`\ 。
3. 右子节点编号：:math:`\text{RightChild}(n)=2n+2`\ 。
...
.. literalinclude:: ../../codes/341_bitree_2.cpp
   :language: c++
...
绝大多数算法问题会给出明确的数据量，假如我们已经知道了最多有可能有 :math:`N` 个节点，而且二叉树的形状并没有特别的控制，那么我们要考虑最坏情况，可能会是 :math:`N` 层，不排除每一层都是最右边的节点的情况，即上面示意图中沿着 ``0->2->6->14`` 这条边一路贴着右边界下来的情况。所以我们需要准备一个长度为 :math:`2^N-1` 的底层数组，:math:`O(2^n)` 级别的空间复杂度，太可怕了！算法竞赛一般要求内存使用不超过128M，那么最大可能的节点数大概为25个以内，一旦 :math:`N` 超过30，普通的家用电脑就没得玩了，如果 :math:`N` 等于64，那么把全世界的内存加在一起也不够了。

所以如果不控制二叉树的形状，顺序结构几乎就是没有任何实用价值的。要用顺序结构，就必须控制二叉树的形状。

.. important::

   顺序结构实现的二叉树，一般都要求是完全二叉树，或总是非常接近完全二叉树。高级数据结构\ :strong:`堆`\ 就是一个非常典型的例子。
