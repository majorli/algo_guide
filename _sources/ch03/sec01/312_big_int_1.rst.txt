...
如果可以让 ``BigInt`` 也能像内置数据类型一样使用IO流输入输出的话，它就更加完美了。为了达到这个目的，我们可以重载C++的IO流运算符 ``<<`` 和 ``>>``\ 。它们是属于IO流的运算符，要重载它们需要引入 ``iostream`` 库。为了在输出的时候能够控制输出格式，还需要引入 ``iomanip`` 库（流操纵库）。

由于它们俩并不是属于 ``BigInt`` 的运算符，而是输出流 ``ostream`` 和 ``istream`` 的，所以重载函数不定义在结构内部。重载函数的原型如下：
...
.. note::
   ...
   ``ostream`` 和 ``istream`` 是C++输出流和输入流的基础类型（基类），所有实际的输入输出流都是从它们两个继承而来的，比如最常见的标准输入输出流 ``cin`` 和 ``cout``\ ，还有文件流、字符串流等。所以重载了这两个运算符之后，不光我们可以实现用 ``cin >>`` 和 ``cout <<`` 来从键盘输入、向屏幕输出 ``BigInt`` 型数据，而且我们同时实现了读写文件等其他所有方式的 ``BigInt`` 数据输入输出。这一点非常棒！
...
3. 使用 ``left``、``right`` 这两个算子，可以控制下一次输出时数据的对齐方式，左对齐或右对齐，默认是右对齐；
