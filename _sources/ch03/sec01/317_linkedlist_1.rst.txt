...
:strong:`链表`\ 采用链式存储结构来实现线性表。既然是线性表，那么元素在逻辑关系上一定是一个接着一个连成一串的，用精确的定义语言来描述就是：每一个元素都至多有一个唯一确定的前驱，也至多有一个唯一确定的后继。但是和顺序表不同，链表中的元素在实际的物理存储上并不要求一个接一个存放，链表可以在任意内存位置随意存放各个元素。为了维持元素之间逻辑上的前驱后继关系，链表为每一个元素附加上表示其前后关系的指针，指向前驱元素的指针称作\ :strong:`前链`\ ，指向后继元素的指针称作\ :strong:`后链`\ 。元素的值和它的前后链组合在一起形成一个\ :strong:`元素节点`\ ，简称\ :strong:`节点`\ ，节点才是链表的基本存储单元，所有元素的节点通过前后链串接起来形成链表。
...
其中红色的箭头线表示后链，蓝色的表示前链。但是这样的示意图看起来非常不舒服，一般我们把链表的示意图画成下面这样更为抽象但是更简洁易懂的方式，图中符号 ∧ 表示空指针： 
...
.. attention::
   ...
   如果给结构提供了一个构造器，而且这个构造器函数是需要提供参数的，那么就必须同时再提供一个不需要参数的默认构造器，否则这个结构类型就不能用于数组。

   不需要参数的默认构造器函数可以是本身就没有参数的函数，也可以是所有参数都有默认值的函数。

一般数据结构教程中都会使用模板类型，我们也一样，后面将介绍怎样实现支持任意数据类型的链表。但是在实际编程解决算法问题时，元素的数据类型一般都是明确的，所以往往并不需要使用模板，将数据类型写死就可以了。
...
.. note::
   ...
   链表使用一个包装过的节点结构来存放元素和链，每一个元素要使用超过自身长度的存储空间，但是总的存储空间和元素的个数还是成正比例的，链表的总体空间占用量仍然是 :math:`O(n)` 级别。
...
.. note::
   ...
   链表只能沿着链的方向迭代遍历节点，无法根据下标直接访问指定位置上的元素，需要按下标访问时要从头节点开始逐个向后寻找，这是一个线性时间 :math:`O(n)` 的操作。
...
.. hint::
   ...
   把一种数据结构的底层存储模式和基本操作功能封装隐藏起来，对外只提供一些规定好的功能函数，这种设计模式就是计算机科学中所称的 :strong:`抽象数据类型`\ （ADTs）。数据结构就是研究各种ADTs的一门学科。

以上所述就是链表最常见的两种形态：单链表和双链表。作为一种非常灵活的线性表数据结构，链表还可以有许多其他变种，比如把首尾相接起来形成所谓的\ :strong:`循环链表`\ 。循环单链表中尾节点的后继不再是空指针而是指向头指针；循环双链表中尾节点的后继指向头节点，头节点的前驱指向尾节点。这样一来，表被改造成了一个环，从而也就没有了严格意义上的头尾节点，所以这种循环链表中我们通常会用一个\ :strong:`当前指针`\ （current）来代替原来的头尾指针。如下图所示：
...
单链表只需要一个头部哑节点，这个哑节点在链表创建的时候就会生成好，链表的头指针就始终指向它。链表中第一个实际的元素节点是这个头部哑节点的后继，当链表为空时，头部哑节点的后继为空指针。
...
单链表只有后继链，所以要删除某个节点，只能通过它的前驱节点来完成。例如要删除节点 ``p`` 的后继结点，那么用一个临时指针指向它的后继，然后将 ``p`` 的后继改成自己后继的后继，最后通过那个临时指针删除掉实际的节点就可以了。

双链表可以由要删除的节点 ``p`` 本身来发起删除：先把 ``p`` 的前驱的后继改成 ``p`` 的后继，然后把 ``p`` 的后继的前驱改成 ``p`` 的前驱，这样 ``p`` 就从链中被游离出来了，直接删掉它就可以了。

和插入操作类似，无论是什么形式的链表，删除操作都是固定步数可以完成的。如果链表有头尾的哑节点，那么对首尾节点的删除操作也就会变得和对其他位置节点删除一模一样。

**按元素下标增删**

到这里，链表的节点插入、删除操作已经介绍完了，我们已经知道了链表是怎样实现常数时间的节点增删。但是链表这个快速的增删操作是有前提条件的，必须事先找到操作的位置。所以我们就会发现一个问题：要删除某个节点，就必须首先找到这个节点，但如果要删除的节点是以元素下标的形式给出的，那么就首先要花费 :math:`O(n)` 时间来找到它（或它的前驱）。

:emphasis:`链表按元素下标增删操作的时间复杂度为` :math:`O(n)`\ 。

因此在实际应用中，链表一般只提供头部增删、尾部增删和指定节点处增删，不提供指定下标位置增删的功能。STL库的双链表容器 ``list`` 就是这样设计的。事实上，链表最擅长的是在表头表尾处增删查改，比如接下来我们要学习的栈和队列。如果某个程序总是要不断按位置随机访问中间段的元素，那就需要斟酌一下到底是用链表还是用线性表了。

.. attention::

   由于单链表的特殊结构，要在单链表尾部增删元素就必须从头到尾先遍历一遍整个链以找到尾节点。当然这个问题可以通过增加一个尾节点指针来解决，但是这个尾节点指针需要单独维护，会增加麻烦。而且由于单链表的链是从前向后单向通行的，所以给单链表增加尾部哑节点毫无意义。

   所以单链表一般来说不提供在表尾增删查改的功能！如果程序总是需要在尾部增加删除元素，那么请使用双链表。

链表 vs 顺序表
^^^^^^^^^^^^^^

下面我们用表格的形式对线性表的两种存储结构，链表和顺序表，进行小结和pk。

+------------------------------------+-----------------------------------------+-------------------------------------------+
|                                    | 链表                                    | 顺序表                                    |
+====================================+=========================================+===========================================+
| 空间复杂度                         | :math:`O(n)`                            | :math:`O(n)`                              |
+-------------+----------------------+-----------------------------------------+-------------------------------------------+
|             | 按下标访问元素       | :math:`O(n)`                            | :math:`O(1)`                              |
|             +----------------------+-----------------------------------------+-------------------------------------------+
| 时间复杂度  | 插入元素             | :math:`O(1)`                            | :math:`O(n)`                              |
|             +----------------------+-----------------------------------------+-------------------------------------------+
|             | 删除元素             | :math:`O(1)`                            | :math:`O(n)`                              |
+-------------+----------------------+-----------------------------------------+-------------------------------------------+
| 实现难度                           | 比较复杂                                | 非常简单                                  |
+------------------------------------+-----------------------------------------+-------------------------------------------+
| 灵活性                             | 强                                      | 一般                                      |
