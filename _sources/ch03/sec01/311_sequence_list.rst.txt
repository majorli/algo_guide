...
这里有一条重要的惯例，就是把那些不允许外部代码访问的成员变量和成员函数，用下划线 ``'_'`` 开头来命名。这是在JavaScript和Python语言中通行的做法。当你遇到一个结构，发现里面有成员变量或成员函数的名字是用下划线开头的，那么就请自觉做到不要去直接访问或调用它们。例如我们的 ``List`` 结构，如果遵循这种惯例的话，应该像下面这样来定义：
...
除了用下标访问元素外，\ ``vector`` 和所有其他STL容器都还支持使用迭代器（iterator）来访问元素。和C++ string一样，各类STL容器都提供了一头一尾两个预设的迭代器，分别用 ``begin()`` 和 ``end()`` 两个成员函数来获得。头迭代器指向首元素；尾迭代器指向紧跟在尾元素后面的那个位置。容器迭代器的基本用法可以参考前面介绍 ``algorithm`` 库排序函数的章节：:ref:`ref-2562`\ 。
